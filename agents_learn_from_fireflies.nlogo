...
state        ;; if the neighbors are yellow (1) or not (0)
done
last-rew
blinking
]
...
globals [radius start_agents n_flashing_fireflies n_flashing_agents n_successful_ticks]
...
to setup
  ...
  ask fireflies [ setxy random-xcor random-ycor ;; spawns fireflies in random locations
      set shape "butterfly"
      set clock random (round cycle-length)
      set threshold flash-length  ;; so the flashing of the fireflies lasts flash-length ticks (slider)
  ...
  ask Walkers [
    qlearningextension:state-def-extra ["state" "blinking"] [bla]
    (qlearningextension:actions [resetLight] [doNothing])
    qlearningextension:reward [rewardFunc]
    qlearningextension:end-episode [isEndState] resetEpisode
    qlearningextension:action-selection "e-greedy" [0.8 0.95]
  ...
  set radius 1
  set start_agents false
  set n_successful_ticks 0
end

;; if we're using the delay strategy (switch in interface), the window is set to -1, this means that
;; a firefly can always reset its clock because we will never have clock <= window

to go

  ask fireflies [
    move  ;; fireflies move randomly
    increment-clock
    if ( (clock > window) and (clock >= threshold) )
      [ look ] ;; if there are enough flashing fireflies around right now, we reset the clock, so the firefly will be synchronized with them
  ]
  ask fireflies [
    recolor
  ]
  ...
  ask Walkers [
    ...
    if print?[
        type "State: " type state type ", Action : " type done type " Reward: " type last-rew type " Blinking?: " type blinking type " --> Clock is now " print clock
        print(qlearningextension:get-qtable)
    ]
  ]
  ]

  set n_flashing_fireflies count fireflies with [color = yellow]
  if n_flashing_fireflies = number [
    set start_agents true
    ;debug_fireflies
  ]
  set n_flashing_agents count walkers with [color = green]
  ;if n_flashing_agents = agents [debug_agents]
  if((n_flashing_fireflies = 0 and n_flashing_agents = 0) or (n_flashing_fireflies = number and n_flashing_agents = agents)) [set n_successful_ticks n_successful_ticks + 1]
  tick
end

to debug_fireflies
  type "Fireflies round:" print ticks
end
to debug_agents
  type "Agents round:" print ticks
end

to-report rewardFunc
  let rew -50
  if (state = 1 and blinking = 1) [set rew 1]
  if (state = 0 and blinking = 0) [set rew 1]
...
to recolor-agent
  if breed = walkers [
   ifelse (clock < threshold)
    [ show-turtle
      set color green set blinking 1]
    [ set color red set blinking 0
      ifelse show-dark-fireflies?
        [ show-turtle ]
        [ hide-turtle ] ]
  ]
...
to look ; turtle procedure
  if count turtles in-radius radius with [color = yellow] >= flashes-to-reset
    [ set clock reset-level ]
end

to look-agents
  ifelse count turtles in-radius 3 with [color = yellow] >= 1
  [set state 1] ;; it means that there's at least 1 neighbor with state = 0
  [set state 0]
end
...
PENS
"flashing" 1.0 0 -1184463 true "" "plot n_flashing_fireflies"
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot n_flashing_agents"
