...
globals [radius n_flashing_agents]
...
to setup
  ...
  ask Walkers [
    ...
    qlearningextension:action-selection "e-greedy" [0.8 0.9955]
    qlearningextension:learning-rate 0.3
    qlearningextension:discount-factor 0.95
    set reward-list []
  ]
...
to go
  ...
  ;if n_flashing_agents = agents [print ticks]
  tick
end

to-report rewardFunc
  let rew neigh
  if blinking = 0 [set rew rew * -2]
  set reward-list lput rew reward-list
  set last-rew rew
  report rew

  ;let rew -5
  ;if (state = 1 and blinking = 1) [set rew 50]
  ;set reward-list lput rew reward-list
  ;set last-rew rew
  ;report rew
...
to look-agents
  set neigh count walkers in-radius agents_radius with [color = green]
  ifelse neigh >= 1
